...
window.addToGoogleCalendar = async function() {
    ...
    const raceDate = document.getElementById('raceStartDate')?.value || new Date().toISOString().split('T')[0];
    const raceTimeRaw = document.getElementById('raceStartTime')?.value || '09:00';
    let startTime = new Date(`${raceDate}T${raceTimeRaw}`);
    if (Number.isNaN(startTime.getTime())) {
        startTime = window.previewData?.startTime || new Date();
    }
    ...
    const weatherText = window._venueWeather?.status === 'ready' && window._venueWeather?.data
        ? `\nWeather: ${window._venueWeather.data.temp}°C, ${window._venueWeather.data.weatherText}, wind ${window._venueWeather.data.wind} km/h`
        : '';
    ...
    const event = {
        ...
        description: `Generated by Streger${raceLocation ? `\nVenue: ${raceLocation}` : ''}${weatherText}`,
